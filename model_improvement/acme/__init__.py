"""Build Phase 5 Acme model-improvement artifacts.
...
from datasets.acme import DEFAULT_OUT as DATASET_DIR
from datasets.acme import build_dataset, write_dataset
...
DEFAULT_OUT = ROOT / "model_improvement" / "acme"
DEFAULT_DATASET_DIR = DATASET_DIR

SYSTEM_PROMPT = (
    "You are the Acme Expense Agent. Answer with the structured final-answer "
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build Phase 5 Acme model-improvement artifacts.")
        ...
        examples.append(
            {
                ...
                "labels": row["labels"],
                "provenance": {
                    "source_dataset": "datasets/acme/cleaned.jsonl",
...
def load_failure_distribution() -> dict[str, Any]:
    path = ROOT / "evals" / "operations" / "acme" / "failure-distribution.json"
    if path.exists():
...
def build_lora_config(sft_metrics: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "dataset": {
            "train": "model_improvement/acme/sft-train.jsonl",
            "dev": "model_improvement/acme/sft-dev.jsonl",
...
def build_gateway_plan() -> dict[str, Any]:
    return {
        ...
        "aliases": {
            "primary": "acme-local-primary",
            "fallback": "acme-hosted-fallback",
            "agent": "acme-agent-model",
        },
        "drills": [
